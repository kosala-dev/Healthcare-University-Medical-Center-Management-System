...
export default function Navbar({ role, userId }) {
  ...
  const Navigation = () => {
    const { designation } = useAuthCheck();
  }
  return (
    <nav className="w-full h-14 flex flex-wrap justify-between items-center px-4 md:px-20 bg-[#670047] text-white">
      <div className="nav-contains w-full md:w-1/2 text-[16px] font-sans font-bold">
        <ul className="flex space-x-12">
          ...
          <li>
            {(designation === 'Doctor' || designation === 'SuperAdmin') && (
                <Link to="/inventory/doctor">View Inventory</Link>
            )}
          </li>
          <li>
            {(designation === 'Nurse' || designation === 'SuperAdmin') && (
                <>
                    <Link to="/inventory/manage">Manage Drugs & Stock</Link>
                    <Link to="/dispense">Dispense Medication</Link>
                    <Link to="/dispense/report">Usage Report</Link>
                </>
            )}
          </li>
        </ul>
