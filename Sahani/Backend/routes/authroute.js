...
router.post("/login", async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { username: user.username || user.regnum, role: roleName || user.designation && { designation: user.designation } },
      roleKey,
      { expiresIn: "1h" }
    );
...
router.post("/registerAdmin", async (req, res) => {
  ...
  if (!username || !gender || !admintype || !password || !designation) {
  ...
  try {
    ...
    const newAdmin = new adminmodel({
      ...
      designation, // 🆕 NEW: Save the designation
    });
