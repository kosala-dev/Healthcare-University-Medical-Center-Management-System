...
// get current logged-in patient by token identity (email or regnum)
router.get("/me", verifyPatient, async (req, res) => {
  try {
    const identifier = req.username;
    const patient = await patientmodel.findOne({
      $or: [{ email: identifier }, { regnum: identifier }],
    });
    ...
    return res.status(200).json({ success: true, patdet: patient });
  } catch (error) {
