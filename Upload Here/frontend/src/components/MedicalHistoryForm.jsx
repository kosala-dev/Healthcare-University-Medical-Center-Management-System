...
export default function MedicalHistoryForm() {
  ...
  const [drugsList, setDrugsList] = useState([]); // All available drugs
  const [selectedDrugs, setSelectedDrugs] = useState([]); // [{drugId, quantity}]
  const [errorMessage, setErrorMessage] = useState("");
  const [successMessage, setSuccessMessage] = useState("");

  const navigate = useNavigate();

  // Fetch all drugs from backend
  useEffect(() => {
  ...
  // Filter out rows without a selected drug or quantity 0
  ...
  try {
    ...
    if (response.status === 201) {
      // 2️⃣ Update drug quantities in DB
      for (const d of validDrugs) {
        try {
          const drug = drugsList.find((dr) => dr._id === d.drugId);
          if (!drug) continue;

          const newQty = drug.quantity - d.quantity;

          await axios.put(`http://localhost:8080/drugs/${d.drugId}`, {
            name: drug.name,
            dosage: drug.dosage,
            quantity: newQty,
          });
        } catch (err) {
          console.error(`Failed to update drug ${d.drugId}:`, err);
        }
      }

      // 3️⃣ Clear form & show success
      ...
      <div className="w-3/4 p-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Patient info fields */}
            {["regNo", "bloodPressure", "bloodSugar", "weight", "temperature","symptoms", "diagnosis"].map(
              (field) => (
                <div key={field}>
