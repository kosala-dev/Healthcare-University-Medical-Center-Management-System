...
async function checkLowStock(drug) {
  if (drug.quantity < 100) {
    // Check if a notification already exists for this drug
    ...
    // Stock >= 100, remove existing notification
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    // Remove any notifications for this drug
    await Notification.deleteMany({
      type: "low_stock",
      message: { $regex: new RegExp(deletedDrug.name, "i") },
    });
