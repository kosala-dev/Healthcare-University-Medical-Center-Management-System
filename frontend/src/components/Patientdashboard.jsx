...
export default function PatientDashboard({ regnum }) {
  ...
  const [resolvedRegnum, setResolvedRegnum] = useState("");

  // get patient details based on logged-in identity (email/regnum)
  useEffect(() => {
    const fetchPatientDetails = async () => {
      try {
        ...
        if (meRes.data.success) {
          setPatientDetails(meRes.data.patdet);
          setPatientDetailsTemp(meRes.data.patdet);
          setResolvedRegnum(meRes.data.patdet.regnum || "");
          return;
        }
      } catch (error) {
        if (regnum && !String(regnum).includes("@")) {
          try {
            ...
            if (fallbackRes.data.success) {
              ...
              setPatientDetailsTemp(fallbackRes.data.patdet);
              setResolvedRegnum(fallbackRes.data.patdet.regnum || "");
              return;
            }
          } catch (fallbackError) {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row">
      <div className="w-full md:w-1/4 bg-gradient-to-r from-[#4A0033] via-[#670047] to-[#9A006C] p-6 md:p-8 flex flex-col justify-between">
        <div>
          <h1 className="text-x1 md:text-2xl font-bold text-white">
      ...
      <div className="w-full md:w-3/4 p-4 md:p-8">
        ...
        <div className="bg-white p-4 md:p-6 rounded-lg shadow-md mb-6">
          <h2 className="text-lg md:text-xl font-bold text-gray-800">
            Registration Number: <span className="text-blue-600">{resolvedRegnum || "Loading..."}</span>
          </h2>
        </div>
        ...
        <div className="flex flex-col sm:flex-row flex-wrap gap-2 mt-6">
          <button onClick={toggleMedicalHistories} className="bg-[#670047] text-white px-4 py-2 rounded-lg transition-all">
            {showMedicalHistories ? "Hide Medical Histories" : "View Medical Histories"}
          </button>
          <button onClick={handleAppointmentClick} className="bg-[#670047] text-white px-4 py-2 rounded-lg transition-all">
            Add Appointment
          </button>
          <button onClick={navigateMedicalForm} className="bg-[#670047] text-white px-4 py-2 rounded-lg transition-all">
            Submit Medical Form
          </button>
          <button
            onClick={goPatientForms}
            className="bg-[#670047] text-white px-6 py-2 rounded-lg "
          >
            Check Approval
          </button>
        </div>
