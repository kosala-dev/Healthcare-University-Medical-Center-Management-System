...
export default function MedicalFormsAdvisor() {
  ...
  const faculty = location.state?.faculty || sessionStorage.getItem("faculty");
  const department = location.state?.department || sessionStorage.getItem("department");
                    ...
                    <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            {f.subjects
                              ...
                              .map((s, idx) => (
                                <li key={idx} className="flex justify-between text-sm">
                                  <span className="font-medium text-gray-800">{s.subject} <span className="text-gray-400">({s.code})</span></span>
                                  
                                  <span className="text-gray-500">{s.dateOfExam ? new Date(s.dateOfExam).toLocaleDateString() : "N/A"}</span>
                                </li>
                              ))}
                          </ul>
                        </div>
                      ...
                      <div className="flex flex-col justify-between border-t lg:border-t-0 lg:border-l border-gray-100 pt-6 lg:pt-0 lg:pl-6">
                        ...
                        <div className="space-y-3 mt-auto">
                          <p className="text-xs text-center text-gray-400 mb-2">Take Action as Advisor</p>
                          <button
