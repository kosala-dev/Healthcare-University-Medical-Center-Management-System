...
export default function MedicalFormsDean() {
  ...
  // FIX: Default to "any" if faculty is missing or null
  const faculty = localStorage.getItem("faculty") || "any"; 
  ...
  const fetchForms = async () => {
    setLoading(true);
    try {
      console.log(`🔍 Dean Fetching: Role=Dean | Faculty=${faculty}`); 
  ...
  const getStatusBadge = (role, status) => {
    let colorClass = "bg-gray-100 text-gray-600"; // Pending/Default
  ...
  const filteredForms = forms.filter((f) => 
    ...
    f.name.toLowerCase().includes(searchTerm.toLowerCase()) // Optional: Search by name too
  );
                  ...
                  >
                    ...
                    <div className="bg-blue-50 p-4 border-b border-blue-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-2">
                      ...
                      <div className="text-sm text-right hidden md:block">
                        <p className="font-semibold text-blue-800">{f.faculty}</p>
                      </div>
                    </div>

                    <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
                      
                      {/* Column 1: Exam & Reason Details */}
                              ...
                              .map((s, idx) => (
                                <li key={idx} className="flex justify-between text-sm">
                                  <span className="font-medium text-gray-800">
                                    {s.subject} <span className="text-gray-400">{s.code ? `(${s.code})` : ""}</span>
                                  </span>
                                  <span className="text-gray-500">
                                    {s.dateOfExam ? new Date(s.dateOfExam).toLocaleDateString() : ""}
                                  </span>
                                </li>
                              ))}
                          </ul>
                      ...
                      <div className="flex flex-col justify-between border-t lg:border-t-0 lg:border-l border-gray-100 pt-6 lg:pt-0 lg:pl-6">
                        ...
                        <div className="space-y-3 mt-auto">
                          <p className="text-xs text-center text-gray-400 mb-2">Take Action as Dean</p>
                          <button
