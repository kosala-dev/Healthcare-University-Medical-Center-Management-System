...
export default function DrugInventory() {
  ...
  const handleDelete = async (drugId) => {
    if (window.confirm("Are you sure you want to delete this drug?")) {
      try {
        const response = await fetch(`http://localhost:8080/drugs/${drugId}`, { method: "DELETE" });
        if (response.ok) setDrugs(drugs.filter((drug) => drug._id !== drugId));
        else console.error("Failed to delete drug");
      } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-gradient-to-r from-blue-50 to-purple-50">
      ...
      {/* Main Content */}
      <div className="flex-1 p-4 md:p-6 lg:p-8 max-w-full md:max-w-5xl mx-auto bg-white rounded-lg shadow-md">
        {/* Header and Buttons */}
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {sortedDrugs.map((drug, index) => (
                <tr
                  key={index}
                  className={`hover:bg-blue-50 transition-colors ${selectedDrug === index ? "bg-blue-100" : ""}`}
                  onClick={() => setSelectedDrug(selectedDrug === index ? null : index)}
                >
                  <td className="px-4 py-2 whitespace-nowrap">{drug.name}</td>
                  <td className="px-4 py-2 whitespace-nowrap">{drug.dosage}</td>
                  <td className="px-4 py-2 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getQuantityColor(drug.quantity)}`}>
                      {drug.quantity} units
                    </span>
                  </td>
                  <td className="px-4 py-2 whitespace-nowrap text-right flex flex-col sm:flex-row justify-end gap-2">
                    <button
