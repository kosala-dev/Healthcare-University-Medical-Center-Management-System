...
export default function AcademicAdvisorDashboard() {
  ...
  // 🔹 Get username from location OR sessionStorage
  const username =
  ...
  useEffect(() => {
    // 🔴 If no username at all → session really expired
    if (!username) {
    ...
    // 🔹 Save username for future page navigations
  ...
  const goAdvisorApproval = () => {
    // 🔴 Prevent clicking before data is ready
    if (!advisorDetails) {
