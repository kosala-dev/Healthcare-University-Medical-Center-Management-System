...
export default function MedicalForm() {
  ...
  const extraDocsRef = useRef(null);
  const historySectionRef = useRef(null);
  const location = useLocation();
  const [pdfReferenceNo, setPdfReferenceNo] = useState("");
  ...
  useEffect(() => {
    const initializePage = async () => {
      ...
      const stateReferenceNo = getReferenceNo(location.state);
      
      let currentRegNum =
        stateRegNum ||
        localStorage.getItem("registrationNo") ||
        localStorage.getItem("regnum") ||
      ...
      if (stateReferenceNo) {
        setPdfReferenceNo(stateReferenceNo);
      }
    };
  ...
  const handleSubmit = async () => {
    try {
      const requiredFields = [
        "name", "address", "contact", "registrationNo", "field", "faculty", 
        "department", "examTitle", "reason", "studentRequest", "firstAttempt"
      ];
      
      const fieldLabels = {
        name: "Name",
        address: "Address",
        contact: "Contact T.P No",
        registrationNo: "Registration No",
        field: "Field of specialization",
        faculty: "Faculty",
        department: "Department",
        examTitle: "Title of Examination",
        reason: "Reason for absence",
        studentRequest: "Request by the student",
        firstAttempt: "First attempt selection",
      };
      ...
      if (missingFields.length > 0 || !form.otherDocs) {
        const missingLabels = missingFields.map((f) => fieldLabels[f] || f);
        if (!form.otherDocs) missingLabels.push("Medical Certificate PDF");
        alert(`Please complete: ${missingLabels.join(", ")}`);
        return;
      }
      ...
      Object.keys(form).forEach((key) => {
        if (key === "subjects") {
          const filteredSubjects = form.subjects.filter(
            (s) => s.subject.trim() !== "" && s.code.trim() !== "" && s.dateOfExam.trim() !== ""
          );
          
          if (filteredSubjects.length === 0) {
            alert("Please fill at least one subject detail in section 7.");
            throw new Error("Validation Error: No subjects provided");
          }
          
          formData.append(key, JSON.stringify(filteredSubjects));
        } else if (key === "agreeToDeclaration") {
        } else if (key === "signature") {
           formData.append(key, form.name);
      ...
      const submitRes = await axios.post("http://localhost:8080/api/submitMedicalForm", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      const exactReferenceNo =
        getReferenceNo(submitRes.data) ||
        getReferenceNo(submitRes.data?.form) ||
        getReferenceNo(location.state);

      if (exactReferenceNo) {
        setPdfReferenceNo(exactReferenceNo);
      ...
      if (printRef.current) {
        ...
        const opt = {
          margin: 0.5,
          filename: `MedicalForm_${regNo}${exactReferenceNo ? `_${exactReferenceNo}` : ""}_${Date.now()}.pdf`,
      ...
      setForm((prev) => ({
        ...
        medicalOfficerName: "", medicalOfficerTitle: "", slmcRegNo: "", 
        otherDocs: null, extraDocs: null,
        agreeToDeclaration: false, 
      }));

      if (fileInputRef.current) fileInputRef.current.value = "";
      if (extraDocsRef.current) extraDocsRef.current.value = "";

    } catch (error) {
      console.error(error);
      if (error.message !== "Validation Error: No subjects provided") {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50 py-10 px-4">
        <div id="pdf-content" ref={printRef} className="relative w-full max-w-4xl mx-auto bg-white shadow-lg rounded-lg p-4 md:p-6">
          {pdfReferenceNo && (
            <p className="absolute top-4 right-6 text-xs font-semibold border border-gray-400 rounded px-2 py-1 bg-white">
              Reference No: {pdfReferenceNo}
            </p>
          )}
          <h1 className="text-xl md:text-2xl font-bold mb-4 text-center mt-5">University Of Vavuniya</h1>
          ...
          <h3 className="mb-6 text-center text-gray-700 text-xs md:text-sm">
            (Duly completed applications should be submit over to the SAR/AR of the respective faculty with relevant documents and medical certified by the University Medical Officer)
          </h3>

          <form className="space-y-6">
            <div className="space-y-3">
              <div>
                <label className="block font-medium">1) Name of the candidate(with initials): Mr./Ms. *</label>
                <input name="name" value={form.name} onChange={handleChange} className="w-full border-b px-2 py-1 mt-2" required />
              </div>
              <div>
                <label className="block font-medium">2) Address *</label>
                <input name="address" value={form.address} onChange={handleChange} className="w-full border-b px-2 py-1 mt-2" required />
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block font-medium">3) Contact T.P No *</label>
                  <input name="contact" value={form.contact} onChange={handleChange} className="w-full border-b px-2 py-1 mt-2" required />
                </div>
                ...
                <div>
                  ...
                  <input 
                    ...
                    readOnly={Boolean(form.registrationNo)}
                    className={`w-full border-b px-2 py-1 mt-2 ${form.registrationNo ? "bg-gray-50 text-gray-600 cursor-not-allowed" : "bg-white text-gray-800"}`} 
                ...
                <div>
                  ...
                  <input name="field" value={form.field} onChange={handleChange} className="w-full border-b px-2 py-1 mt-2" required />
                </div>
              </div>
              ...
              <div>
                ...
                <input name="examTitle" value={form.examTitle} onChange={handleChange} className="w-full border-b px-2 py-1 mt-2" required />
              </div>
              ...
              <div>
                ...
                <textarea name="reason" value={form.reason} placeholder="Reason" onChange={handleChange} rows={2} className="w-full border px-2 py-1" required />
              </div>

              <div>
                <label className="block font-medium">9) Request by the student *</label>
                <input name="studentRequest" value={form.studentRequest} onChange={handleChange} className="w-full border-b px-2 py-1 mt-2" required />
              </div>

              <div>
                <label className="block font-medium">10) Whether this is your first attempt for the above examinations? If "No" give details *</label>
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-2">
                  {["Yes", "No"].map((type) => (
                    <label key={type} className="flex items-center gap-2">
                      <input type="radio" name="firstAttempt" value={type} checked={form.firstAttempt === type} onChange={() => setForm({ ...form, firstAttempt: type })} />
                      {type}
                    </label>
                  ))}
                </div>
                <textarea name="firstAttemptDetails" placeholder="If No, explain here..." onChange={handleChange} rows={2} className="w-full border px-2 py-1" />
              </div>

              <div className="mt-4">
                <label className="font-medium">Medical Certificate (Attach PDF) *</label>
                <input type="file" name="otherDocs" accept="application/pdf" ref={fileInputRef} onChange={handleChange} className="block mt-2 w-full text-sm" required />
                {form.otherDocs && <p className="text-sm text-gray-600 mt-1">Selected file: {form.otherDocs.name}</p>}
              </div>

              <div className="mt-4">
                <label className="font-medium">Any other documents, if available (Attach PDF)</label>
                <input type="file" name="extraDocs" accept="application/pdf" ref={extraDocsRef} onChange={handleChange} className="block mt-2 w-full text-sm" />
                {form.extraDocs && <p className="text-sm text-gray-600 mt-1">Selected file: {form.extraDocs.name}</p>}
              </div>
              ...
              <div>
                System generated application by medical center of vavuniya on {new Date().toLocaleDateString()}
              </div>
            </div>
          </form>
        </div>
      ...
      <div className="flex flex-col items-center justify-center gap-4 mt-6 px-4">
        <button 
            ...
            className={`w-full max-w-sm px-6 py-3 rounded-lg text-white font-bold text-lg shadow-md transition-all ${
                ...
                ? 'bg-[#670047] hover:bg-[#670047] hover:shadow-lg transform hover:-translate-y-0.5' 
